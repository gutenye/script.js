  ...
  describe('completion spec', () => {
    test('generates spec file with auto name', () => {
      ...
      const specsDir = path.join(
        os.homedir(),
        'Library/Application Support/carapace/specs',
      )
...
function runDirect(akeFile: string, cwd: string, ...args: string[]) {
  return spawn(['bun', akeFile, ...args], cwd)
}

function runViaAke(cwd: string, ...args: string[]) {
  return spawn(['bun', akeBinary, ...args], cwd)
}
