...
- **Entry:** `src/index.ts` exports `app` (Command) and `$` (shell execution). Registers `process.on('beforeExit')` to auto-run `app.run()`; an explicit `await app.run()` removes that listener and `run()` remains idempotent
