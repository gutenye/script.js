...
```ts
PROJECT_DIR  // absolute path to the project dir, only available inside command actions. You may not need it since the action's cwd is already the project dir
```
